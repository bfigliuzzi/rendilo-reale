...
/**
 * Pool SoA de particules d'effets (morts, impacts, ramassages, boss) + screen shake.
 * Même contrat que les autres pools : zéro allocation au runtime, swap-remove,
 * `Particle` index-verrouillées, garées hors champ quand mortes.
 *
 * Décliné de `games/horde/src/render/fx.ts` : le repo ne mutualise un module dans
 * `shared/` qu'à son 2e consommateur, et les deux jeux divergeront (Berceau ajoutera
 * de la gravité aux gerbes de bave).
 */
export class Fx {
  private count = 0;
